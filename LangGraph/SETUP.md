...
- `langgraph_env`: Name of your environment (you can choose another name if desired).
...
You’ll know it’s activated when `(langgraph_env)` appears at the start of your command line.
...
`python -m ipykernel install --user --name=langgraph_env --display-name "Python (langgraph_env)"`
...
`Kernel → Change Kernel → Python (langgraph_env)`
...
**Project:** Langchain Course / Lab Setup
